...
function set_fake_h(h) {
	fake_h.style.height = h + "px";
}
function setScroll(num) {
	home_scroll.style = `transform: translateY(${num * -1}px);`;
}
...
window.addEventListener("scroll", () => {
	...
	if (newScroll > obj1H * (1 / 3)) {
		ani2(nowScroll)
	}
	if (newScroll > obj1H * (1 / 2)) {
		ani3(nowScroll)
	}
...
function ani2(s) {
	...
	if (s > obj1H) {
		console.log('a');
		moveY = ani2T1_s + (s - (obj1H * 1 / 3)) * 1;
	} else {
		console.log('b');
		moveY = ani2T1_s + (s - (obj1H * 1 / 3)) * 1.2;
	}
