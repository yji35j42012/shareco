...
var home_seven=document.querySelector("#home_seven");
var home_txtBox=document.querySelector("#home_txtBox");
var fake_h=document.querySelector("#fake_h");
...
var moveSpeed=0.85
var home_scroll=0
var range3=window_h/moveSpeed
var range4=window_h/moveSpeed
var range5=window_h/moveSpeed
var setFake_h=window_h+range3+range4+range5


var second_s=0, third_s=100, forth_s=100
var thirdT1_s=-75, thirdT2_s=-60, thirdT_op=0


var st_count=0, st_count2=0, st,wheelNew=0
...
function mouseHandle(e) {
	html.scrollTop-oldS>0? home_scroll=1:home_scroll=-1
	console.log('home_scroll', home_scroll);
	if (html.scrollTop>=parseInt(range3)&&st_count<=2&&home_scroll>0) {
		html.scrollTop=range3
		clearTimeout(st)
		scrollTime()
	}else if(html.scrollTop<parseInt(range3)&&st_count>0){
		html.scrollTop=range3
		clearTimeout(st)
		scrollTime()
	}

	if (html.scrollTop>=parseInt(range3)+parseInt(range4)&&st_count2<=2&&home_scroll>0) {
		html.scrollTop=range3+range4;
		clearTimeout(st)
		scrollTime2()
	} else if (html.scrollTop<parseInt(range3)+parseInt(range4)&&st_count2>0) {
		html.scrollTop=range3+range4;
		clearTimeout(st)
		scrollTime2()
	}
...
function home2() {
	var m=(100-(range3-html.scrollTop)/range3*100)*-1;

	home_second.style.transform=`translateY(${ m }%)`;
}
function home3() {
	var m=(range3-html.scrollTop)/range3*100;
	var op=1-m*2/98
	// console.log('m', m);
	if (m<50) {
		var m1=m*2/101*thirdT1_s
		var m2=m*2/103*thirdT2_s
		third_txt1.style.transform=`translateY(${ m1 }%)`;
		third_txt2.style.transform=`translateY(${ m2 }%)`;
		third_txt1.style.opacity=op
		third_txt2.style.opacity=op
	}
	if (m>50&&op!==0) {
		third_txt1.style.opacity=op
		third_txt2.style.opacity=op
	} else if (m<0) {
		third_txt1.style.opacity=2-op
		third_txt2.style.opacity=2-op
	}
	home_third.style.transform=`translateY(${ m }%)`;
	home_txtBox.style.transform=`translateY(${ m }%)`;
}
function home4() {
	var m=(range3+range4-html.scrollTop)/range4*100;
	// console.log('home4', m);
	home_forth.style.transform=`translateY(${ m }%)`;
}

document.addEventListener("mousewheel", mouseHandle, false);
document.addEventListener("scroll", mouseHandle, false);

function homeInit() {
	fake_h.style.height=setFake_h+"px";
	home_second.style=`transform: translateY(${ second_s }%);transition: all ${ ani_dus }s;`;
	home_third.style=`transform: translateY(${ third_s }%);transition: all ${ ani_dus }s;`;
	home_txtBox.style=`transform: translateY(${ third_s }%);transition: all ${ ani_dus }s;`;
	third_txt1.style=`transform: translateY(${ thirdT1_s }%);transition: all ${ ani_dus }s;opacity: ${ thirdT_op };`;
	third_txt2.style=`transform: translateY(${ thirdT2_s }%);transition: all ${ ani_dus }s;opacity: ${ thirdT_op };`;
	home_forth.style=`transform: translateY(${ forth_s }%);transition: all ${ ani_dus }s;`;


	home_fifth.style.display="none"
	home_six.style.display="none"
	home_seven.style.display="none"
}
