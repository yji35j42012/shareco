...
var changePageTime;
var ispc=true
var nowD=window.innerWidth<1024? ispc='ph':ispc='pc'
...
var sev_txt_ph=document.querySelector("#sev_txt_ph");
var sevT_op=0, sevT_m=100, sevPhT_m=100, sevPhT_op=0
...
function forthCatHandler() {
	...
	forthCat.style=`transform: translateY(${ forthCat_m }%);transition: all ${ forthCat_d }s;`;
}
function forthMove() {
	var t=(range3-home_y)/rangeSpeen
	if (t<40) {
	...
	forthBg.style=`transform: translateY(${ forthBg_m }%);transition: all ${ forthBg_d }s;opacity: ${ forthBg_op };`;
	forthTxt1.style=`transform: translateY(${ forthT_m }%);transition: all ${ forthT_d }s;opacity: ${ forthT_op };`;
}
function forthOut() {
...
function sevMove() {
	...
	sev_txt_ph.style=`transform: translate3d(0,${ sevPhT_m }%,0);opacity: ${ sevPhT_op };transition: opacity transform 0.2s;`;
...
function homeInit() {
	...
	sev_txt_ph.style=`opacity: ${ sevT_op };transition: opacity 0.2s;`;
...
function pcShow() {
	...
	sev_txt.classList.remove("d_none");
}
function phShow() {
	...
	sev_txt_ph.classList.remove("d_none");
}
...
window.onresize=function () {
	if (window.innerWidth<1024&&nowD=='pc') {
		...
		fifth_video_m.style.display='';
		fifth_video_m.play()
		fifth_video.style.display="none"
		phShow()
	} else if (window.innerWidth>=1024&&nowD=='ph') {
		ispc=true;
		nowD='pc'
		fifth_video_m.style.display='none';
		fifth_video.style.display=""
		fifth_video.setAttribute("src", "../images/home/blacksmoke.mp4")
		pcShow()
	}
